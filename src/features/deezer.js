...
// $(document).on('click', '#deezerSubmit', () => getAlbumData($('#deezerAlbum').val(), $('#deezerArtist').val()));
...
$(document).on('click', '#deezerSubmit', async () => { 
// const searchQuery = $('#searchTest').val();
//getAlbumData(serachQuery)
const album = await getAlbumData('Take Me Back To Eden', 'Sleep Token');
let obj = '';
let tracks = '';
...
album.tracks.forEach((track) => {
  tracks += `<li>${track.id} - ${track.title}</li>`;
})
